...
<style>
  div.container { width: 700px; margin: 20px auto; font-size: 11pt; }
  form { padding: 10px 40px 40px 40px; width: 400px; 
    box-shadow: 0px 0px 4px rgba(0, 0, 0, 0.4), 2px 2px 4px rgba(0, 0, 0, 0.4);}
  ...
  .btn { padding: 0.7em 1.5em; border: 1px solid #aaa; border-radius: 4px; 
  ...
  .btn:active { -ms-transform: translateY(2px); -webkit-transform: translateY(2px);
        transform: translateY(2px); ba ckground: #ccc;  }
  div.info { margin-top: 20px; padding: 10px 20px; background-color: #dff; border: 1px solid #8ff; }
  div.error { margin-top: 20px; padding: 10px 20px; background-color: #fdd; border: 1px solid #faa; }
</style>
</head>
<body>
<div class="container">
...
<form method="post">
  ...
  <div>
    ...
    <select name="departmentId">
      <option value="1" <%= student.getDepartmentId() == 1 ? "selected" : "" %>>소프트웨어공학과</option>
      <option value="2" <%= student.getDepartmentId() == 2 ? "selected" : "" %>>컴퓨터공학과</option>
      <option value="3" <%= student.getDepartmentId() == 3 ? "selected" : "" %>>정보통신공학과</option>
      <option value="4" <%= student.getDepartmentId() == 4 ? "selected" : "" %>>글로컬IT공학과</option>
    </select>
  </div>
  <div>
    <label>학년</label>
  ...
  <% if (에러메시지 != null) { %>
    <div class="error">저장 실패: <%= 에러메시지 %></div>
  <% } %>
</form>
